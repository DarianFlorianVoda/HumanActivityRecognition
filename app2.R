...
server <- function(input, output){
  ...
  # Box Plot
  output$Box_Plot <- renderPlot({motion_data_box <- select(motion_data_scale, c("Acceleration.X","Acceleration.Y","Acceleration.Z","AngularVelocity.X","AngularVelocity.Y","AngularVelocity.Z","Orientation.X","Orientation.Y","Orientation.Z"))
  boxplot(motion_data_box) + scale_x_discrete(guide = guide_axis(angle = 90))
  })
  # Correlation Plot
  output$Correlation_Plot <- renderPlot({# Calculate the correlation matrix of the data frame
    cor_matrix <- cor(motion_data_all_numeric)
      ...
      tabItem(tabName = "EDA",
              fluidRow(
                ...
                box(
                  ...
                  ,solidHeader = TRUE 
                  ,collapsible = TRUE ,
                  plotOutput("Pair_plot", height = 500))
